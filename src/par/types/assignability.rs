...
use super::core::{PrimitiveType, Type};
...
impl Path {
    ...
    pub fn add_name(&self, name: LocalName) -> Self {
        Path::NamedNode(name, Rc::new(self.clone()))
    }
}

#[derive(Clone, Debug)]
pub struct LabelsMap(pub HashMap<Option<LocalName>, Rc<(Path, Type, FixPointType, LabelsMap)>>);
    ...
    fn is_assignable_to_internal(
        ...
        Ok(match (self, other) {
            (Self::Self_(_, label1), Self::Self_(_, label2)) => {
                if !self_labels.0.contains_key(label1) || !other_labels.0.contains_key(label2) {
                    return Ok(false);
                }
            ...
            (Self::Self_(_, label1), t2) => {
                if !other_labels.0.contains_key(label1) {
                    return Ok(false);
                }
